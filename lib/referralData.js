...
const MAX_CODES_PER_PLATFORM = 2;
...
export async function findReferralPlatform({ platformId, query }) {
  const sql = getSql();
...
export async function getUserReferralStats(userId) {
  ...
  const [row] = await sql`
    select
      ...
      coalesce(sum(usage_count), 0)::integer as usage_count,
      coalesce(sum(report_count), 0)::integer as report_count,
      count(distinct platform_id)::integer as platform_count
    from invite_codes
...
function publicUserInviteCode(row) {
  return {
    id: row.id,
    platformId: row.platform_id,
...
export async function listUserInviteCodes(userId) {
  const sql = getSql();
  const rows = await sql`
    select
      ic.id, ic.platform_id, p.name as platform_name, ic.code, ic.referral_url,
      ic.display_type, ic.status, ic.usage_count, ic.report_count,
      ic.last_used_at, ic.expires_at, ic.created_at
    from invite_codes ic
    join referral_platforms p on p.id = ic.platform_id
    where ic.user_id = ${userId} and ic.status <> 'deleted'
    order by ic.created_at desc
  `;

  return rows.map(publicUserInviteCode);
}

export async function setUserInviteCodeStatus(userId, codeId, status) {
  ...
  const [row] = await sql`
    with updated as (
      ...
      where id = ${codeId} and user_id = ${userId} and status <> 'deleted'
      returning *
    )
    select updated.*, p.name as platform_name
    from updated
    join referral_platforms p on p.id = updated.platform_id
  `;

  return row ? publicUserInviteCode(row) : null;
}

export async function updateUserInviteCode(userId, codeId, { code, expiresAt }) {
  const sql = getSql();
  const [row] = await sql`
    with updated as (
      update invite_codes
      set code = ${code}, expires_at = ${expiresAt ?? null}, updated_at = now()
      where id = ${codeId} and user_id = ${userId} and status <> 'deleted'
      returning *
    )
    select updated.*, p.name as platform_name
...
export async function softDeleteUserInviteCode(userId, codeId) {
  const sql = getSql();
  const [row] = await sql`
    update invite_codes
...
export async function createUserInviteCode(userId, { platformId, platformName, code, expiresAt }) {
  const sql = getSql();
  const platform = await findReferralPlatform({ platformId, query: platformName });
  ...
  const [{ count }] = await sql`
    ...
    from invite_codes
    where user_id = ${userId} and platform_id = ${platform.id} and status <> 'deleted'
  `;

  if (count >= MAX_CODES_PER_PLATFORM) {
    return { error: 'limit_reached' };
  }

  const id = randomUUID();
  const [row] = await sql`
    insert into invite_codes (id, platform_id, category_id, user_id, code, display_type, status, source_type, expires_at)
    values (${id}, ${platform.id}, ${platform.category_id}, ${userId}, ${code}, 'code', 'active', 'user_submitted', ${expiresAt ?? null})
    returning *
  `;

  return { inviteCode: publicUserInviteCode({ ...row, platform_name: platform.name }) };
}
